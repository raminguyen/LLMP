...
def plot_charts_metrics(result1, result2, result3):
    ...
    metrics3 = extract_metrics_for_models(result3)

    # Create a table
    table = PrettyTable()
    table.field_names = [
        "Model", 
        "Result 1 MSE", "Result 2 MSE", "Result 3 MSE",
        "Result 1 MLAE", "Result 2 MLAE", "Result 3 MLAE"
    ]
    ...
    for model in metrics1.keys():
        table.add_row([
            model,
            metrics1[model][0], metrics2[model][0], metrics3[model][0],  # MSEs
            metrics1[model][1], metrics2[model][1], metrics3[model][1]   # MLAEs
        ])
    ...
    result3_mse = [metrics3[model][0] for model in models]
    result1_mlae = [metrics1[model][1] for model in models]
    result2_mlae = [metrics2[model][1] for model in models]
    result3_mlae = [metrics3[model][1] for model in models]

    # Define distinct colors for the three results
    colors = ['b', 'g', 'r']  # Blue, Green, Red

    # Apply stronger jitter to the x-positions to ensure visibility of overlapping points
    def jitter_positions(positions, jitter_amount=0.1):
        jittered_positions = []
        for i, pos in enumerate(positions):
            jittered_positions.append(pos + jitter_amount * (-1 if i % 2 == 0 else 1))
    ...
    plt.subplot(1, 2, 1)
    plt.scatter(jitter_positions(model_positions, -0.1), result1_mse, color=colors[0], label='Result 1 MSE')
    plt.scatter(model_positions, result2_mse, color=colors[1], label='Result 2 MSE')
    plt.scatter(jitter_positions(model_positions, 0.1), result3_mse, color=colors[2], label='Result 3 MSE')
    ...
    plt.subplot(1, 2, 2)
    plt.scatter(jitter_positions(model_positions, -0.1), result1_mlae, color=colors[0], label='Result 1 MLAE')
    plt.scatter(model_positions, result2_mlae, color=colors[1], label='Result 2 MLAE')
    plt.scatter(jitter_positions(model_positions, 0.1), result3_mlae, color=colors[2], label='Result 3 MLAE')
    plt.xticks(model_positions, models, rotation=15)
